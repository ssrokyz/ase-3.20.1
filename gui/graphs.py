...
def make_plot(data, i, expr, type, show=True):
    ...
    if type == 'y':
        for j in range(m):
            plt.plot(data[j], c='k') ## YJ
            plt.plot([i], [data[j, i]], 'o', markersize=5, c='navy') ## YJ
    else:
        for j in range(1, m):
            plt.plot(data[0], data[j], c='k') ## YJ
            plt.plot([data[0, i]], [data[j, i]], 'o', markersize=5, c='navy') ## YJ
